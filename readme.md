...
with advice rooted in real classical texts (verse-level citations coming via RAG, see `PLAN.md`).
...
server reads `GEMINI_API_KEY` from the environment. See `PLAN.md` for the full roadmap
(RAG citations, evals, auth) and the differentiated feature list.
